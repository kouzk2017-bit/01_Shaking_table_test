"""Signal operations matching the archived MATLAB helper functions."""
...
def fft_filter(
    ...
    frequency = np.abs(
        np.fft.fftfreq(mirrored.shape[0], d=1.0 / sampling_frequency)
    )
...
def resample_decimate(
    ...
    if decimation < 1 or not np.isclose(factor, decimation, atol=1e-12):
        raise ValueError("The legacy resampler requires integer decimation")
    if decimation == 1:
        result = values.copy()
    else:
        mirrored = np.concatenate((values, values[::-1]), axis=0)
